...
function GC_ProductionFactory:load(nodeId, xmlFile, xmlKey, indexName, isPlaceable)
	...
	local factoryImage, factoryCamera
	...
	if self.numInputProducts > 0 then
		...
		while true do
			...
			if hasXMLProperty(xmlFile, playerTriggerKey) then
				...
				local playerTrigger = self.triggerManager:addTrigger(GC_PlayerTrigger, self.rootNode, self, xmlFile, playerTriggerKey, nextId, true, activateText)
